...
class RekamMedik_model
{

    private $table = 'tb_redik';
    private $table1 = 'tb_pasien';
    private $table2 = 'tb_pegawai';
    private $table3 = 'tb_layanan';
    private $db;
    ...
    // public function getRedikById($id_redik)
    // {
    //     $this->db->query('SELECT * FROM ' . $this->table . ' WHERE id_redik = :id_redik');
    //     $this->db->bind('id_redik', $id_redik);
    //     return $this->db->single();
    // }

    public function getAllRedik()
    {
        $this->db->query('SELECT * FROM ' . $this->table . ' r' . ' INNER JOIN ' . $this->table1 . ' pas' . ' ON r.id_pasien = pas.id_pasien' . ' INNER JOIN ' . $this->table2 . ' peg' . ' ON r.id_pegawai = peg.id_pegawai' . ' INNER JOIN ' . $this->table3 . ' l' . ' ON r.kode_layanan = l.kode_layanan');
